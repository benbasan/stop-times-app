...
export default function StopLookup({ prefillStop }: StopLookupProps) { // ✅ מקבל prop
  ...
  const { addFavorite } = useFavorites(); // ✅ שימוש ב-Context
  ...
  // ✅ כשבוחרים מועדף (או שמגיע prop), תמלא את השדה ותבצע חיפוש אוטומטי
  React.useEffect(() => {
    if (prefillStop && prefillStop !== stopCode) {
      setStopCode(prefillStop)
      void load(prefillStop)
    }
  }, [prefillStop, stopCode, load])
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="bg-white rounded-2xl shadow p-4 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          {/* ✅ שמירה למועדפים דרך ה-Context (ללא כפולים) */}
          <button
            type="button"
            onClick={() => {
              const id = stopCode.trim()
              if (!id) return
              const name = prompt("תן שם למועדף הזה:", `תחנה ${id}`)
              if (!name) return
              // הימנע מכפולים: פשוט תנסה להוסיף; ה-Provider יכול לסנן, אבל נוסיף גם פה סינון קל
              addFavorite({ id, name })
            }}
            className="px-4 py-2 rounded-xl bg-amber-200 hover:bg-amber-300"
            title="שמור למועדפים"
          >
            ⭐ שמור למועדפים
          </button>
